...
public class TransactionImpl implements TransactionGateway {
    ...
    @Override
    public Page<Transaction> findByScheduledDate(LocalDate scheduledDate, LocalDateTime firstExecution, PageRequest pageable) {
        return transactionRepository.findByScheduledDate(scheduledDate, firstExecution, pageable)
                .map(TransactionEntity::toTransaction);
    }
